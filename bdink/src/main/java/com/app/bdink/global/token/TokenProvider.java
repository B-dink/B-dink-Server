...
public class TokenProvider {
    ...
    private final String kollusKey; // jwt 서명을 위한 비밀 키. 토큰을 생성하고 검증할 때 사용
    ...
    //todo:현재 jwt 토큰 생성에서 문제가 발생. 해결될때 까지 일반 url로 접근
    @Transactional
    public KollusTokenDTO createKollusJwtToken(String userkey, String mediacontentkey) {
        ...
        String kollusAccessToken = Jwts.builder()
                .claim("cuid", userkey) // 사용자 유저키 정보 저장
                .claim("mc", List.of(Map.of("mckey", mediacontentkey))) // 동영상의 미디어 콘텐츠 키 저장
                .setExpiration(kollusAccessTokenValidityTime) // 토큰의 만료 시간 설정
                .signWith(SignatureAlgorithm.HS256, kollusKey.getBytes()) // Kollus측의 보안키를위해서 구버전으로 서명
                .compact(); // 토큰 생성
